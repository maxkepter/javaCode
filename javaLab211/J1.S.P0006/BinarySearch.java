...
public class BinarySearch {
    ...
    public static int binarySearch(int[] arr, int k, int r, int l) {
        if (r <= l) {
            int p = (r + l) / 2;
            if (arr[p] == k) {
                return p;
            } else if (arr[p] < k) {
                return binarySearch(arr, k, r, p - 1);

            } else {
                return binarySearch(arr, k, p + 1, l);
            }
        }
        return -1;
    }
    ...
    public static void main(String[] args) {
        ...
        int index = binarySearch(arr, k, 0, arr.length - 1);
        if (index == -1) {
